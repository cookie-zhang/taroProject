    ...
    render(){
        ...
        return(
            <View className='topicsList-menu' >
                <View className={showDrawerFlage?'leftDrawerContanierActive':'leftDrawerContanier'} onClick={this.handleClose.bind(this,1)}>
                    <View className='leftMenu'>
                        {
                            items.map((item,index)=>{
                                return(
                                    <View onClick={this.handleMenuItem.bind(this,index,'test')} className='menuItem' key={index}>
