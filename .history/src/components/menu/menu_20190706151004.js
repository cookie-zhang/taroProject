...
class Menu extends Component {
    ...
    handleMenuItem(e){
        ...
        console.log(cataData[1])
        let item =cataData[e.target.dataset.eHandlemenuitemAA];
        console.log(item)
                    ...
                    <View className='leftMenu'>
                        {
                            items.map((item,index)=>{
                                return(
                                    <View  className='menuItem' key={item.id} id={item.id} onClick={this.handleMenuItem.bind(this,item,index)}>
